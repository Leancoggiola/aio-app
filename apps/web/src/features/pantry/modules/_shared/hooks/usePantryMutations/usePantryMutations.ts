...
export function usePantryMutations() {
  ...
  const invalidateSummary = useCallback(async () => {
    await mutate(SWR_KEYS.pantry.summary, undefined, { revalidate: true });
  }, [mutate]);

  const invalidateProducts = useCallback(async () => {
    await mutate((key: unknown) => typeof key === 'string' && key.startsWith(SWR_KEYS.pantry.products), undefined, {
      revalidate: true,
    });
  }, [mutate]);

  const invalidateShoppingList = useCallback(async () => {
    await mutate(SWR_KEYS.pantry.shoppingList, undefined, { revalidate: true });
  }, [mutate]);
  ...
  const createProduct = useCallback(
    async (payload: CreatePantryProductPayload) => {
      const product = await api.post<PantryProduct>(SWR_KEYS.pantry.products, payload);
  ...
  const updateProduct = useCallback(
    async (productId: string, payload: UpdatePantryProductPayload) => {
      const product = await api.patch<PantryProduct>(SWR_KEYS.pantry.product(productId), payload);
      await invalidatePantry();
      return product;
    },
    [invalidatePantry]
  ...
  const deleteProduct = useCallback(
    async (productId: string) => {
      await api.delete(SWR_KEYS.pantry.product(productId));
      await invalidatePantry();
    },
    [invalidatePantry]
  );
  ...
  const generateShoppingList = useCallback(async () => {
    const items = await api.post<PantryShoppingListItem[]>(SWR_KEYS.pantry.shoppingListGenerate);
    await invalidatePantry();
    return items;
  }, [invalidatePantry]);
  ...
  const addShoppingListItem = useCallback(
    async (payload: AddShoppingListItemPayload) => {
      const item = await api.post<PantryShoppingListItem>(SWR_KEYS.pantry.shoppingListItems, payload);
  ...
  const completeShoppingListItem = useCallback(
    async (itemId: string, payload: CompleteShoppingListItemPayload) => {
      const result = await api.patch<{ item: PantryShoppingListItem; product?: PantryProduct }>(
        SWR_KEYS.pantry.shoppingListItemComplete(itemId),
        payload
      );
      await invalidatePantry();
      return result;
  ...
  const deleteShoppingListItem = useCallback(
    async (itemId: string) => {
      await api.delete(SWR_KEYS.pantry.shoppingListItem(itemId));
      await Promise.all([invalidateShoppingList(), invalidateSummary()]);
    },
    [invalidateShoppingList, invalidateSummary]
  );
  ...
  const clearCheckedShoppingList = useCallback(async () => {
    await api.delete(SWR_KEYS.pantry.shoppingListChecked);
    await Promise.all([invalidateShoppingList(), invalidateSummary()]);
  }, [invalidateShoppingList, invalidateSummary]);
