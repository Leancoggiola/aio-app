...
export function useGymMutations() {
  ...
  const invalidatePlans = useCallback(async () => {
    await mutate((key: unknown) => typeof key === 'string' && key.startsWith(SWR_KEYS.gym.plans), undefined, {
      revalidate: true,
    });
  }, [mutate]);
  ...
  const invalidatePlan = useCallback(
    async (planId: string) => {
      await mutate(SWR_KEYS.gym.plan(planId), undefined, { revalidate: true });
    },
    [mutate]
  );
  ...
  const createPlan = useCallback(
    async (payload: CreateGymPlanPayload) => {
      const plan = await api.post<GymPlanDetail>(SWR_KEYS.gym.plans, payload);
  ...
  const archivePlan = useCallback(
    async (planId: string) => {
      const plan = await api.post<GymPlanDetail>(SWR_KEYS.gym.planArchive(planId));
  ...
  const addExercise = useCallback(
    async (payload: AddGymExercisePayload) => {
      const plan = await api.post<GymPlanDetail>(SWR_KEYS.gym.exercises, payload);
  ...
  const updateExercise = useCallback(
    async (exerciseId: string, payload: UpdateGymExercisePayload, planId: string) => {
      const plan = await api.patch<GymPlanDetail>(SWR_KEYS.gym.exercise(exerciseId), payload);
      await Promise.all([invalidatePlans(), invalidatePlan(planId)]);
      return plan;
    },
    [invalidatePlan, invalidatePlans]
  ...
  const updateExerciseWeight = useCallback(
    async (exerciseId: string, payload: UpdateGymExerciseWeightPayload, planId: string) => {
      const exercise = await api.patch<GymExercise>(SWR_KEYS.gym.exerciseWeight(exerciseId), payload);
      await Promise.all([invalidatePlans(), invalidatePlan(planId)]);
      return exercise;
    },
    [invalidatePlan, invalidatePlans]
  ...
  const deleteExercise = useCallback(
    async (exerciseId: string, planId: string) => {
      await api.delete(SWR_KEYS.gym.exercise(exerciseId));
      await Promise.all([invalidatePlans(), invalidatePlan(planId)]);
    },
    [invalidatePlan, invalidatePlans]
  );
