...
export function useSplitFriends() {
  const { data, error, isLoading } = useSWR<SplitFriend[]>(API_KEYS.splitExpenses.friends);
  return { items: data ?? [], isLoading, error };
}

export function useSplitFriendsSuggest(q: string, limit = 10) {
  const trimmed = q.trim();
  const key = trimmed ? `${API_KEYS.splitExpenses.friendsSuggest}${buildQueryString({ q: trimmed, limit })}` : null;
...
export function useGatherings(params: Partial<ListGatheringsParams> = {}) {
  ...
  const key = `${API_KEYS.splitExpenses.gatherings}${buildQueryString(query)}`;
...
export function useGathering(gatheringId: string | null | undefined) {
  const key = gatheringId ? API_KEYS.splitExpenses.gathering(gatheringId) : null;
